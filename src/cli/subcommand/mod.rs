...
mod split_receipt {
    ...
    ///
    /// This will produce two modified receipts -- a phase 1 receipt and a phase 2 reciept. If you
    ...
    async fn two_phased_can_parse_receipt_perfectly(
        ...
        tracing::debug!(
            "Using the 'can actually parse receipt perfectly' method to split the receipt"
        );
        ...
        fn roundtrip_to_extract_type<T: serde::de::DeserializeOwned>(
            ...
            let action_unjson: StatefulAction<T> = serde_json::from_str(&action_json)
                .with_context(|| {
            ...
            match inner_typetag_name {
                action_tag if action_tag == crate::action::common::ProvisionNix::action_tag().0 => {
                    let action_unjson = roundtrip_to_extract_type::<
                        crate::action::common::ProvisionNix,
                    >(&action, action_tag)?;
                ...
                action_tag
                    if action_tag == crate::action::base::CreateDirectory::action_tag().0 =>
                {
                    let action_unjson = roundtrip_to_extract_type::<
                ...
                action_tag
                    if action_tag == crate::action::macos::CreateNixVolume::action_tag().0 =>
                {
                    let action_unjson = roundtrip_to_extract_type::<
                ...
                s if s == "create_determinate_nix_volume" => {
...

                    {
                        ...
                        phase2_plan
                            .actions
                            .push(action_obj["encrypt_volume"].clone());
                        // FIXME: WHY IS UNMOUNT VOLUME PROGRESS????? -- because we expect it to fail, and once it fails, it doesn't set the failure status
                        // maybe I fix this later
                        phase2_plan
                            .actions
                            .push(action_obj["unmount_volume"].clone());
                    }
