...
impl Action for CreateOrMergeNixConfig {
    ...
    async fn execute(&mut self) -> Result<(), ActionError> {
        ...
        // FIXME: isn't currently inserting things from existing config that aren't merged
        if let Some(existing_nix_config) = nix_configs.existing_nix_config.as_mut() {
            ...
            let discovered_buf = discovered_buf.split('\n').collect::<Vec<_>>();

            for line in &discovered_buf {
                if line.is_empty() {
                    continue;
                }

                // TODO: handle standalone comments with locality...?

                // Preserve inline comments for settings we've merged
                if let Some((name, value)) = existing_nix_config
                    .settings()
                    .iter()
                    .find(|(name, _value)| line.starts_with(*name))
                {
                    ...
                    if let Some(merged_value) =
                        nix_configs.merged_nix_config.settings_mut().get_mut(name)
                    {
                        *merged_value = format!("{merged_value} {comment}");
                    } else {
        ...
        dbg!(&nix_configs.merged_nix_config);
...
mod test {
    ...
    async fn preserves_comments() -> eyre::Result<()> {
        ...
        assert!(s.contains("# another inline comment"));
        assert!(s.contains("# Generated by"));
        assert!(s.contains("ca-references"));
        assert!(NixConfig::parse_file(&test_file).is_ok());
